...
$my_net_info=trim($_POST['my_net_info']);


if (preg_match('#^([0-9]{1,3}\.){3}[0-9]{1,3}((([0-9]{1,3}\.){3}[0-9]{1,3})|(\/[0-9]{1,2}))$#im',$my_net_info,$matches) && empty($matches)){
	tr("Invalid Input.");
	tr('Use IP & CIDR Netmask:&nbsp;', '10.0.0.1/22');
	tr('Or IP & Netmask:','10.0.0.1 255.255.252.0');
	tr('Or IP & Wildcard Mask:','10.0.0.1 0.0.3.255');
...
if (preg_match("/\//im",$my_net_info,$matches) && $matches){  //if cidr type mask
	...
	$bin_wmask=binnmtowm($bin_nmask);
	if (preg_match("/0/im",rtrim($bin_nmask, "0"),$matches) && (count($matches)>0)) {  //Wildcard mask then? hmm?
...
if (preg_match('/^(00001010)|(101011000001)|(1100000010101000)/im',$bin_net)){
